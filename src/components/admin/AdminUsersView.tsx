...
export const AdminUsersView: React.FC = () => {
  ...
  return (
    <div className="space-y-4 pb-24 sm:pb-8">
      ...
      <AdminAssetSection type="floor_plan" title="국사 평면도 DB" description="국사별 도면을 최대 3장까지 등록하고, 각 도면의 Rack 위치 좌표를 독립적으로 관리합니다." accept=".png,.jpg,.jpeg,.webp" icon={<Building2 className="h-5 w-5" />} assets={floorPlans} onChanged={loadAdminData} />
      <AdminAssetSection type="b2c" title="B2C 선번장 / 직선도 DB" description="선번장 D/H/L~P열을 조회 DB로 교체하고, 직선도 시트는 실제 콘텐츠만 자른 벡터 PDF·정밀 좌표 지도로 생성합니다." accept=".xlsx,.xls,.csv" icon={<Cable className="h-5 w-5" />} assets={b2cAssets} activeStraightMapFilenames={activeStraightMapFilenames} onChanged={loadAdminData} />

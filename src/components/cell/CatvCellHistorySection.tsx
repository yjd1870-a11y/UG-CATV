...
import { Camera, Clock3, History, ImagePlus, Plus, Trash2, Upload, UserRound, X } from 'lucide-react';
...
export const CatvCellHistorySection: React.FC<CatvCellHistorySectionProps> = ({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      const loadedPhotos = await Promise.all(selected.map(readFile));
          ...
          {history.map((item) => (
            <article key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button type="button" onClick={() => void handleDelete(item.id)} className="rounded-lg p-2 text-slate-400 transition hover:bg-red-50 hover:text-red-500" aria-label={`${item.title || '작업이력'} 삭제`}>
              ...
              {item.photos?.length ? (
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {item.photos.map((photo, index) => (
                    <img key={`${item.id}-${index}`} src={photo} alt={`${item.title || '작업이력'} 현장 사진 ${index + 1}`} className="aspect-4/3 w-full rounded-xl border border-slate-200 object-cover" />
                  ))}
                </div>
              ) : null}
            </article>
          ...
          <div className="w-full max-w-xl rounded-3xl bg-white p-5 shadow-2xl sm:p-7">
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-4">
              ...
              <div className="border-t border-slate-200 pt-4">
                ...
                {photos.length < 3 ? (
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-3 flex min-h-28 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white px-4 text-center transition hover:border-[#2878B5] hover:bg-blue-50/30">
                    <Upload className="h-7 w-7 text-slate-400" />
                    <span className="mt-2 text-xs font-bold text-slate-700">클릭하여 사진 첨부 (최대 3장)</span>
